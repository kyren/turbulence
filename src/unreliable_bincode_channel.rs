...
/// to arrive in order.
pub struct UnreliableBincodeChannel {
    channel: UnreliableChannel,
    buffer: Vec<u8>,
    bincode_config: bincode::Config,
}

impl UnreliableBincodeChannel {
    pub fn new(
    ...
    /// Write the given serializable message type, which must serialize to less than
    /// `MAX_MESSAGE_LEN` in size.
    ///
...
pub struct UnreliableTypedChannel<T> {
    channel: UnreliableBincodeChannel,
    _phantom: PhantomData<T>,
}

impl<T> UnreliableTypedChannel<T> {
    pub fn new(
        packet_pool: MuxPacketPool,
        incoming: Receiver<MuxPacket>,
        outgoing: Sender<MuxPacket>,
...
impl<T: Serialize> UnreliableTypedChannel<T> {
    pub async fn send(&mut self, msg: &T) -> Result<(), SendError> {
        self.channel.send(msg).await
    }
}

impl<'a, T: Deserialize<'a>> UnreliableTypedChannel<T> {
...
fn from_inner_send_err(err: unreliable_channel::SendError) -> SendError {
    match err {
        ...
        unreliable_channel::SendError::TooBig => unreachable!(),
    }
}
...
fn from_inner_recv_err(err: unreliable_channel::RecvError) -> RecvError {
    match err {
        ...
        unreliable_channel::RecvError::TooBig => unreachable!(),
    }
}
