...
pub struct MessageChannelsBuilder<S, T> {
    pool: MuxPacketPool,
    spawner: S,
    timer: T,
...
impl<S, T> MessageChannelsBuilder<S, T> {
    pub fn new(pool: MuxPacketPool, spawner: S, timer: T) -> Self {
...
///
/// Acts as a bridge between the sync and async worlds.  Provides sync ethods to send and receive
...
type ChannelTask = BoxFuture<'static, Result<(), Box<dyn Error + Send + Sync>>>;
type RegisterFn<S, T> = fn(
    MessageChannelSettings,
    &mut PacketMultiplexer,
    &mut ChannelBuilder<S, T>,
    &mut ChannelsMap,
) -> ChannelTask;
...
fn register_message_type<S, T, M>(
    settings: MessageChannelSettings,
    multiplexer: &mut PacketMultiplexer,
    builder: &mut ChannelBuilder<S, T>,
