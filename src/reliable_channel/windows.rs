use std::{collections::VecDeque, num::Wrapping, u32};

use crate::wrap_cmp::{wrap_cmp, wrap_ge, wrap_gt, wrap_lt};
...
impl SendWindow {
    ...
    pub fn ack_range(&mut self, start: StreamPos, end: StreamPos) -> AckResult {
        match self
            .unacked_ranges
            .binary_search_by(|(range_start, _)| wrap_cmp(range_start, &start))
        {
            Ok(i) => {
                if wrap_gt(&end, &self.unacked_ranges[i].1) {
...
impl RecvWindow {
    ...
    pub fn recv(&mut self, start_pos: StreamPos, data: &[u8]) -> Option<StreamPos> {
        ...
        if wrap_gt(&start_pos, &recv_end_pos) {
            return None;
        }
        ...
        let copy_start_pos = if wrap_gt(&self.recv_pos, &start_pos) {
            self.recv_pos
        } else {
            start_pos
        };
        ...
        let end_pos = if wrap_lt(&end_pos, &recv_end_pos) {
            end_pos
        } else {
            recv_end_pos
        };
        ...
        if wrap_ge(&copy_start_pos, &end_pos) {
        ...
        if wrap_ge(&self.recv_pos, &start_pos) {
            ...
            let pos = match self
                .unready
                .binary_search_by(|(_, end)| wrap_cmp(end, &end_pos))
            {
                Ok(i) => i,
                Err(i) => i,
            };
            ...
            let insert_pos = match self
                .unready
                .binary_search_by(|(_, end)| wrap_cmp(end, &start_pos))
            {
                Ok(i) => i,
                Err(i) => i,
            };
